...
<!-- skill: meal-plan
needs: cart, corpus
description: Plan meals and build the grocery list for the week. Use when the user wants a menu or to shop — "make me a menu", "let's do groceries", "I'm running low", "I want to make X tonight", "plan dinners for the week" — or seeds the week with new pantry items (a farmers-market haul). Runs the load-context → reason → propose → save flow, then (for a Kroger user) offers ready-to-eat add-ons and to place the order. Captures buy/cook intent and the grocery list; the cart flush and pricing themselves are the order skill, not this one. -->
...
5. **Present the plan and iterate.** Reason over everything plus my original message (freeform constraints like "comfort food one night," "I'm feeling lazy," "something Italian," "date night Thursday" — fold the mood/vibe in naturally, it's reasoning context, not a separate input). Send the proposal in chat and iterate on my revisions, rerunning affected tool calls as needed. The proposal carries:
   - The dinner plan, sized to my cooking frequency — which nights I'll cook vs. nights I'll eat out or grab something easy (concrete heat-and-eat options come in the ready-to-eat pass, step 7, if I'm a Kroger user).
...
7. **Ready-to-eat — a no-cook night (Kroger, configured catalog).** If my store is Kroger and I've set up a ready-to-eat catalog, *after* the cooking plan is saved, offer **1–2** heat-and-eat options that are currently fulfillable (`ready_to_eat_available()`, curbside/delivery) for an easy / eat-out-adjacent night — an order add-on, never a gate. Ask whether to **include any in the order**; on a yes, add them to the grocery list. Skip entirely for a non-Kroger trip or an empty catalog. (On-sale RTE *discovery* and *restock* of low favorites are buy-time concerns — they live in the flush skills, place-grocery-order and shopping-list, not here.)
