...
- **`/health` aggregates them** on the **fetch** path — deliberately, because `fetch` is independent of `scheduled`, so the endpoint stays answerable when the cron is dead, and an external monitor catches a stopped job via stale `last_run_at`. It is **open and tenant-clean** (no per-tenant data; the D1 probe is coarsened to a boolean so no raw `storage_error` string leaks; 200 when ok, 503 when a job is failing). The payload carries nothing secret, so restricting *who* may read it is an **edge** concern (Cloudflare Access / WAF), not Worker code — the same "emit truthful state, decide policy outside" stance as the alerting. (There is no `HEALTH_TOKEN`; the gate was retired with the admin panel.)
