...
- **The last-paid read:** `idx_spend_events_item` serves the per-household "what did we last pay for X" memo — the latest non-voided priced event per `line_key`, a tenant-scoped **query, not a table** (waste valuation and the estimation ladder consume it; never cross-household).
