...
Live inventory. Agent-writable. Updated as side effect of menu generation and ad-hoc messages. Stored as rows in the D1 `pantry` table (`PRIMARY KEY (tenant, normalized_name)`; columns `name`, `normalized_name`, `quantity`, `category`, `prepared_from`, `added_at`, `last_verified_at`, `notes`; `idx_pantry_category(tenant, category)` backs the `read_pantry` category filter). `notes` is an optional short freeform string (added in `migrations/d1/0007_pantry_notes.sql`). Previously the `state:<username>:pantry` JSON array in `DATA_KV`, before that `users/<username>/pantry.toml` in GitHub; migrated into rows once, at deploy time, by a one-time deploy backfill (since retired). Adds are `INSERT … ON CONFLICT DO UPDATE` (keep `added_at`, refresh `last_verified_at`, overlay the rest); reads/writes are row-level and strongly consistent. The schema below describes each item object's shape:
...
prepared_from = null
notes = "freezer burned, best for stocks or stews"   # optional short freeform note
