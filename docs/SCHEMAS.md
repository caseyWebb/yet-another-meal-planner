...
## Cookbook query-vector cache (KV, not a repo file)

Derived, ephemeral state written by the cookbook's hybrid search (`src/cookbook.ts` + `src/cookbook-search.ts`) into the `KROGER_KV` namespace. Documented here for completeness; nothing edits it by hand, and a miss simply re-embeds.

- `cookbook:qvec:<hash>` → a JSON `number[]` of `EMBED_DIM` floats: one search query's embedding vector. `<hash>` is `hashText(EMBED_MODEL + "\n" + normalizeQuery(q))`, so the key is bound to the embedding **model** (a model change re-keys — never a stale-dimension reuse) and case/whitespace-equivalent queries collapse to one entry. Written with a **30-day TTL** on a cache miss; a parse/shape mismatch on read is treated as a miss, not an error. **Anonymous and cross-tenant** — the cookbook has no caller identity, and a query vector carries no tenant data. Caching the *vector* (not the result list) means a corpus reconcile is reflected on the next search with no invalidation.
