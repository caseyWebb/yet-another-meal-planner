...
- *(Optional)* **README status badge.** With `HEALTH_TOKEN` as a `var` **and** the `WORKER_HOST` repo variable set, the Deploy workflow stamps the Worker's `/health.svg` card into your data-repo README (inside a `<!-- health-badge -->` block) and re-stamps it each deploy. It refreshes on a short TTL when the README is viewed — a glance, not an alarm (keep the monitor below for alerting). **No `contents: write` on your `deploy.yml`?** The deploy can't commit it, but it **always prints the ready-to-paste snippet in its run summary** — copy that `![grocery-mcp health](…/health.svg?token=…)` line into your README once (token + host are stable, so it's a one-time paste).
