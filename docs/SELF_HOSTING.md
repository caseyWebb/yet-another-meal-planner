...
- Set a **`HEALTH_TOKEN`** (any random string) — either a Worker secret, or (recommended if you want the README badge below) a plaintext **`var`** in your `wrangler.jsonc` (`"vars": { …, "HEALTH_TOKEN": "…" }`). It only gates this tenant-clean endpoint and your data repo is private, so keeping it in `vars` is fine — and it lets the deploy read it to stamp the badge. `/health` is **disabled (404) until it's set**; then call `https://<your-worker-host>/health?token=<HEALTH_TOKEN>` — JSON (`200` healthy, `503` when a job is failing).
- *(Optional)* **README status badge.** With `HEALTH_TOKEN` as a `var` **and** the `WORKER_HOST` repo variable set, the Deploy workflow stamps the Worker's `/health.svg` card into your data-repo README (inside a `<!-- health-badge:start -->` / `<!-- health-badge:end -->` block) and re-stamps it each deploy. It refreshes on a short TTL when the README is viewed — a glance, not an alarm (keep the monitor below for alerting). **No `contents: write` on your `deploy.yml`?** The deploy can't commit it, but it **always prints the ready-to-paste snippet in its run summary** — copy that `![grocery-mcp health](…/health.svg?token=…)` line into your README once (token + host are stable, so it's a one-time paste).
