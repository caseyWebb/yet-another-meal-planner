...
**Params:**
- `update_preferences`: `patch` (object, required) — a **JSON Merge Patch (RFC 7396)** over the caller's preferences. `stores` supports `{primary, preferred_location, location_zip, nicknames:{[store_slug]: string}}`; nicknames are household-private presentation and never write the shared `stores` row. Other defined keys are `cadence`, `planning_cadence_days`, `weekly_budget`, `brands`, `dietary`, `rotation`, and `custom`; nested objects merge and `null` deletes.
