...
- `nudges` (object, optional):
  ...
  - `proteins?` (string[]): a week-level **soft** protein boost — matching candidates get a bounded additive bump and a `why` line; never a gate (the per-slot `protein` pin is the hard version).
