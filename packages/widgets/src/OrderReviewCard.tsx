...
export function OrderReviewCard({
	...
	const base = resolveOrderReviewCapabilities({
	...
	React.useEffect(() => {
		if (
			!base.contractSupported ||
			host?.serverTools == null ||
			host?.updateModelContext == null
		)
			return;
		let cancelled = false;
		void bridge
			.callServerTool({
				name: "read_order_review",
				arguments: {
					stage: {
						skipped: [],
						quantities: {},
			...
			.then((result) => {
				const review = orderReviewFromBridge(result);
				if (!cancelled && review) {
					setCurrent(review);
					setHydrated(true);
	...
	const capabilities = resolveOrderReviewCapabilities({
	...
	const adapter = React.useMemo(
		() =>
			createOrderReviewBridgeAdapter(bridge, capabilities, () => {
				void bridge.requestTeardown?.({});
			}),
		[bridge, capabilities.mode, capabilities.contractSupported],
	);
	return (
		<div data-widget="order-review" data-hydrated={hydrated || undefined}>
			{bootFailed ? (
				<p role="status">
					Showing the saved review read-only; current facts could not be
					refreshed.
				</p>
			) : null}
			<OrderReview data={current} adapter={adapter} onDataChange={setCurrent} />
		</div>
	);
}
