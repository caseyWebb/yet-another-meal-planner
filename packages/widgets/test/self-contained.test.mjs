// Build-target check (recipe-card-widget, spec: "Self-contained, zero-external-request widget
// bundle"). Asserts the emitted widget HTML is ONE self-contained document — all JS + CSS
// inlined, and NO external resource-loading construct (external stylesheet link, external
// `<script src>`, `@font-face`, `@import url(http…)`, `url(http…)` in CSS, or a webfont host).
//
...
//
// The package `test` script runs `vite build` first, so the artifact is fresh when this runs.
...
const htmlPath = path.resolve(here, "../../worker/assets/widgets/recipe-card.html");

test("the widget bundle is emitted", () => {
  assert.ok(existsSync(htmlPath), `expected built widget at ${htmlPath} (run \`vite build\` first)`);
});

test("the widget bundle is self-contained (zero external resource requests)", () => {
  ...
  assert.ok(html.includes("recipe-card-widget"), "expected the recipe-card-widget marker");
});
