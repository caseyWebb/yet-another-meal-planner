...
describe("Order Review MCP bridge", () => {
  ...
  it("mirrors full context without messages for drafts/save and announces one successful send", async () => {
    ...
    const sendMessage = vi.fn(async (_params: Parameters<OrderReviewBridge["sendMessage"]>[0]) => undefined);
    const bridge: OrderReviewBridge = {
      updateModelContext,
      sendMessage,
      callServerTool: vi.fn(async ({ name }) => {
        if (name === "save_order_brand_preference")
          return {
            structuredContent: {
              status: "saved",
              family_key: "milk",
              brand: "A",
              family: { tiers: [["A"]], any_brand: false },
        ...
        if (name === "place_order")
          return {
            structuredContent: {
              ...
              steps: {
                list: { advanced: true },
                cart: { written: true, count: 1 },
                send: { recorded: true, id: "s", item_count: 1, estimated_total: 2, flyer_savings: 0 },
                cache: { committed: true, inserted: ["milk"], updated: [], unchanged: [] },
              },
              left_off: [],
    ...
    const context: OrderReviewModelContext = {
      preview: data,
      stage: data.stage,
      save_receipts: [],
      action_summary: "quantity changed",
    };
    ...
    await adapter.saveBrand({
      ...
      line_key: "milk",
      brand: "A",
      expected_family_fingerprint: "f0",
      preview_fingerprint: "p",
      stage: data.stage,
    });
    expect(sendMessage).not.toHaveBeenCalled();
    const send = await adapter.send({ stage: data.stage, preview_fingerprint: "p", cleared_cart_ack: false });
    await adapter.notifyCompletion?.({ send, save_receipts: [], carted_names: ["Milk"] });
    expect(updateModelContext).toHaveBeenCalledWith({ structuredContent: context });
    expect(sendMessage).toHaveBeenCalledTimes(1);
    expect(sendMessage.mock.calls[0]?.[0].content[0]?.text).toContain("Milk");
  });

  it("delegates the complete stage and uses host teardown for Back to grocery", async () => {
    const sendMessage = vi.fn(async (_params: Parameters<OrderReviewBridge["sendMessage"]>[0]) => undefined);
    const requestTeardown = vi.fn(async () => undefined);
    const bridge: OrderReviewBridge = {
      callServerTool: vi.fn(async () => ({ structuredContent: data as unknown as Record<string, unknown> })),
      updateModelContext: vi.fn(async () => undefined),
      sendMessage,
      requestTeardown,
    };
    const adapter = createOrderReviewBridgeAdapter(bridge, { mode: "delegate", contractSupported: true });
    const stage = {
      ...data.stage,
      skipped: ["milk"],
      quantities: { eggs: 3 },
      impulses: [{ key: "i", label: "ice" }],
    };
    await adapter.delegate?.({ kind: "send", stage, cleared_cart_ack: true });
    const text = sendMessage.mock.calls[0]?.[0].content[0]?.text ?? "";
    expect(text).toContain('"skipped":["milk"]');
    expect(text).toContain('"cleared_cart_ack":true');
