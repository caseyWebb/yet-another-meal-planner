...
// over the shared tenant-scoped Spend and Waste analyzers. The routes accept no
// public tenant selector and perform no direct storage access or write.
...
import type { WasteRange } from "../waste-shapes.js";
import { jsonWithEtag } from "./etag.js";

const RETROSPECTIVE_RANGES: readonly SpendRange[] = ["4w", "8w", "12w"];
...
export const retrospectiveArea = new Hono<ApiEnv>()
  .get("/retrospective/spend", requireSession, async (c) => {
    ...
    if (!(RETROSPECTIVE_RANGES as readonly string[]).includes(rawRange)) {
  ...
  .get("/retrospective/waste", requireSession, async (c) => {
    const rawRange = c.req.query("range") ?? "8w";
    if (!(RETROSPECTIVE_RANGES as readonly string[]).includes(rawRange)) {
      throw new ToolError("validation_failed", "range must be 4w | 8w | 12w");
    }
    ...
    return jsonWithEtag(
      c,
      await readWasteAnalyzer(
        c.env,
        tenant.id,
        rawRange as WasteRange,
        c.req.query("mapping_version"),
      ),
    );
  });
