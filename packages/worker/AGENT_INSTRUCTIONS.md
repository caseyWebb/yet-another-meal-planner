...
4. **Flush.** Once I've dispositioned the batch, call `place_order` for real — pass `overrides` for the items I picked SKUs for, `include_partials` for the partials I confirmed, `quantities` for anything beyond 1 package, and `exclude` for any line I said to skip this order ("don't buy the salmon this time" — a derived line has no row to remove, and the exclusion is order-scoped, never persisted). Resolved items advance to `in_cart`.
