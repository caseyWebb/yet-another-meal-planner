...
export class GroceryPage extends AppPage {
  ...
  async deactivateInCart(): Promise<void> { await this.page.evaluate(async () => { const { items } = await (await fetch("/api/grocery")).json() as { items: { name: string; status: string }[] }; for (const item of items) if (item.status === "in_cart") await fetch(`/api/grocery/items/${encodeURIComponent(item.name)}`, { method: "PATCH", headers: { "content-type": "application/json", "X-App-Csrf": "1" }, body: JSON.stringify({ status: "active" }) }); }); }
  ...
  async openOrder(): Promise<void> { await this.page.getByTestId("order-open").click(); }
  orderPanel(): Locator { return this.page.getByTestId("order-panel"); }
  orderLine(name: string): Locator { return this.orderPanel().locator(`[data-testid="order-line"][data-name="${name}"]`); }
  async excludeLine(name: string): Promise<void> { await this.orderLine(name).getByTestId("order-exclude").click(); }
  async setLineQty(name: string, quantity: number): Promise<void> { await this.orderLine(name).getByTestId("order-qty").locator("input").fill(String(quantity)); }
  checkpointItem(name: string): Locator { return this.orderPanel().locator(`[data-testid="order-checkpoint-item"][data-name="${name}"]`); }
  async pickCandidate(name: string, sku: string): Promise<void> { await this.checkpointItem(name).locator(`[data-testid="order-cand"][data-sku="${sku}"]`).check(); }
  partialRow(name: string): Locator { return this.orderPanel().locator(`[data-testid="order-partial"][data-name="${name}"]`); }
  async confirmPartial(name: string): Promise<void> { await this.partialRow(name).getByTestId("order-partial-confirm").check(); }
  staleWarning(): Locator { return this.page.getByTestId("order-stale-warning"); }
  async ackStaleCart(): Promise<void> { await this.page.getByTestId("order-stale-ack").check(); }
  commitButton(): Locator { return this.page.getByTestId("order-commit"); }
  async commitOrder(): Promise<void> { await this.commitButton().click(); }
  resultCart(): Locator { return this.page.getByTestId("order-result-cart"); }
  resultList(): Locator { return this.page.getByTestId("order-result-list"); }
  relinkButton(): Locator { return this.page.getByTestId("order-relink"); }
}
