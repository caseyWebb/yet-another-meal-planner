...
- [ ] **Scraper version.** A PR touching `packages/scraper/**` or the shared `packages/contract/**` bumped `packages/scraper/package.json` `version` (strictly greater) — enforced by the `scraper-version` gate (or no scraper/contract change).
