...
- [ ] 1.1 Add `packages/worker/src/night-vibe-dedupe.ts` (pure, no I/O — the
...
- [ ] 1.2 `packages/worker/test/night-vibe-dedupe.test.ts` (vitest, off-`workerd`): synthetic
...
- [ ] 2.1 `packages/worker/src/reconcile-db.ts`: widen `PendingProposal["status"]` to
...
- [ ] 2.2 Extend `packages/worker/test/reconcile.test.ts`: `supersedeProposals` flips only
...
- [ ] 3.1 `packages/worker/src/night-vibe-suggest.ts` — compose per design D7: gate the
...
- [ ] 3.2 `runArchetypeDerivationJob`: fold `superseded` into the health/run summary and the
  usage counts (`{ members, enqueued, superseded }`).
- [ ] 3.3 Tests: a `night-vibe-suggest` suite (vitest + `test/fake-d1.ts`, `vi.mock` the
...
- [ ] 3.4 `aubr typecheck` + `aubr test` green.

## 4. Docs (same pass, lockstep)

- [ ] 4.1 `docs/TOOLS.md` — `suggest_night_vibes`: the phrase-space dedup guarantee (palette,
  ...
  covers superseded.
- [ ] 4.2 `docs/SCHEMAS.md` — `pending_proposals.status`:
...
- [ ] 4.3 `docs/ARCHITECTURE.md` — the archetype-derivation paragraph (§ the generate half):
...
- [ ] 5.1 `openspec validate "dedupe-night-vibe-suggestions"` passes; PR from the template
