...
- **`hidden` boolean — NO (reuse what exists).** A per-tenant "never show me this" is already covered: per-tenant `status: rejected` (via `set_recipe_status`) hides a corpus recipe from one member's active set while keeping it for de-dup, and the group-wide `reject_discovery` suppresses a URL before import. A third `hidden` column would duplicate `rejected` with no distinct semantics, so the overlay stays `{ favorite, status }` (+ the inert legacy `rating`). YAGNI-consistent with the codebase's "add only when measured" posture.
- **Favorite cutover scope — favorite swap landed, draft-retirement deferred.** The `rating → favorite` swap (overlay column + backfill, `toggle_favorite` + the split-off `set_recipe_status`, `COUNT(favorite)` group signal, the re-rank favorites repoint, the user-tunable `rotation` prefs) shipped as one change. Retiring the `draft` state so imports land `active` (the disposition collapse) is its own behavioral flip for the production flow and is deferred to a focused follow-up; until then the `status` lifecycle (`active`/`draft`/`rejected`) is unchanged and `create_recipe` still defaults to `draft`.
