...
- [ ] 2.1 `admin/src/Route.elm`: add a `Health` variant; map `top` and `s "admin"` → `Health`; keep `s "admin" </> s "members"` → `Members`; update `toString`/`href` (`Health → /admin`); refresh the module doc.
- [ ] 2.2 `admin/src/Main.elm`: add `HealthPage Status.Model` to `Page` and `HealthMsg` to `Msg`; handle them in `update`, `enter`, and `stepTo`; make `init` land on the `Health` route.
- [ ] 2.3 `admin/src/Main.elm`: render a three-link nav (Status / Members / Dev · Tools) with active-state helpers, route `HealthPage` through `viewPage`, and update the module doc to describe the three areas.

## 3. Styling

- [ ] 3.1 Add status-view styles to `admin/index.html` (state colors for healthy / failing / never-run, row layout, headline), reusing existing `.card` / `.error` classes where they fit.
...
- [ ] 5.1 Update `admin/CLAUDE.md` (and grep for any other prose enumerating the panel as just "Admin + Dev areas") to include the Status home area.

## 6. Build & validate

- [ ] 6.1 `openspec validate admin-status-homepage --strict` passes.
- [ ] 6.2 Rebuild the bundle with `aubr build:admin` and confirm `aubr build:admin --check` is clean; if `package.elm-lang.org` is unreachable, leave the rebuild to CI and do **not** commit a stale `admin/dist/`.
- [ ] 6.3 Run the admin Elm tests and `aubr test` / `aubr typecheck` as a regression check; confirm green.
