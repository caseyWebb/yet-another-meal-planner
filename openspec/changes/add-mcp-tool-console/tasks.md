...
- [x] 6.3 Elm: the `NoPersona | Acting` and `Page` unions make the key invariants ("no persona ⇒ no invoke"; no page-holds-another's-state) **compile-enforced** — verified by `aubr build:admin`. Added a focused `elm-test` harness for the *compiler-opaque* logic: `admin/tests/RouteTest.elm` (16 cases — `fromUrl`, trailing slash, `toString`↔`fromUrl` round-trip, `actingAsParam` incl. empty/substring/percent-decoded) and `admin/tests/ToolConsoleTest.elm` (the `needsConfirm` run-gate safety convention). Run via `aubr test:admin` (`scripts/test-admin.mjs`, classic elm-test over npx like build-admin), wired into CI. Also **hardened** `actingAsParam` to use `Url.Parser.Query` (tested library splitting/decoding) instead of a hand-rolled string split, and extracted the run-gate predicate to the exposed `ToolConsole.needsConfirm` so the safety convention has one tested source of truth.
