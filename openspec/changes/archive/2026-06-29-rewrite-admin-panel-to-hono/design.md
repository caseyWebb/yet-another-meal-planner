...
### 8. Full SSR navigation (no client-side router)
Navigation between areas is **full SSR** — no client-side router, no client route state (the SPA complexity the rewrite sheds). **Alternative rejected:** *a client-side router* — reintroduces SPA route state + bundle for no real gain on a one-operator tool. **Tried and dropped:** *cross-document View Transitions* (`@view-transition { navigation: auto; }`) — the animation read as *slower* (an extra paint between navigations) without earning its keep, so the opt-in + the shell `view-transition-name`s were removed. Plain instant SSR navigation it is.
