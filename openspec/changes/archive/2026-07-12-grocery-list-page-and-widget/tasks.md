...
- [x] 1.1 Add a D1 migration for `grocery_list.checked_at`, `row_version`, and `updated_at`, `order_sends.placed_at`, and the tenant-scoped substitution and pantry-coverage decision tables/indexes; add migration tests proving old rows read as unchecked and old sends remain unplaced.
...
- [x] 3.3 Implement persistent substitution accept/undo with attribution-signature invalidation and edited-replacement safety; retain taste-capture behavior and add virtual/explicit/undo tests.
- [x] 3.4 Implement Buy-anyway/Undo coverage decisions and bind Still good to pantry verification, returning full post-write snapshots and preserving independently edited rows.
...
- [x] 4.1 Implement send-scoped Back to list with row-version/send-membership guards, linkage clearing, immutable snapshot retention, no-spend guarantees, and idempotent offline replay tests.
- [x] 4.2 Implement atomic `mark_grocery_send_placed` membership validation, exact batch `in_cart → ordered`, `placed_at`, and D16 writer invocation; cover mismatch all-or-nothing, cross-tenant/send rejection, zero-line rejection, replay, and no re-pricing.
...
- [x] 5.4 Extend member component/browser coverage for deterministic grouping, virtual check, offline/reload replay, pantry actions, substitution/Undo, two send groups, quote honesty, exact mark-placed conflicts, per-line relist, aging, household, and unknown/unlinked states; capture/review changed screenshots and run `aubr test:app`.
...
- [x] 6.3 Implement the MCP host adapter with frozen first paint, contract/capability gates, D19 boot re-hydration, server-tool writes, immediate full `ui/update-model-context`, mark-placed-only completion messaging, sendMessage delegation, and read-only degradation.
- [x] 6.4 Add fixture-driven MCP adapter/widget tests for every action channel, conflict/`isError` silence, cached-payload rehydrate, unknown-newer version, capability ladder, no credentials, plain-text parity, and self-contained bundle output.
