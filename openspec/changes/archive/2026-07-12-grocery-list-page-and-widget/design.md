...
The current route’s staged exclude is host-local and cannot survive D19 boot. Add `grocery_substitution_decisions` keyed by `(tenant, original_key)` with `replacement_key`, the original recipe-attribution signature, decision revision/operation claims, and a server-stamped ownership token shared with a row created by that decision. The shared accept operation atomically claims the decision, materializes or reactivates the replacement, and merges attribution without replacing independent row fields; the decision suppresses the original from the shopping algebra. Creation is established by the successful write, never by a pre-write existence check. Undo compare-and-swaps the observed decision and removes the replacement only when the same decision owns an untouched row; otherwise it preserves the row and reports that outcome. A concurrent decision change makes stale Undo conflict without deleting newer state.
...
**Still good** calls `mark_pantry_verified`, re-reads, and mirrors the full snapshot. **Buy anyway** atomically materializes or reactivates the canonical row as `source:'pantry_low'` with a note that it overrode coverage and records a buy-anyway decision so that pantry subtraction does not immediately hide it. The decision and a row it creates share an atomic ownership token. Undo compare-and-swaps the decision and removes only its untouched owned row; an ordinary racing add, an independent edit, or a newer decision survives. Pure section collapse is local-only.
...
Add `relist_grocery_send_line(send_id, line_key, expected_row_version)`, where `send_id` is nullable. It conditionally performs `in_cart → active`, clears `sent_in`, retains quantity, writes no spend, and leaves historical send snapshots immutable. A non-null id must match the row's current unplaced tenant send. Null is the guarded fallback for any row proven outside all current unplaced tenant sends, including absent, dangling, placed, or otherwise unmatched linkage; it conflicts if the row still belongs to an open send. Relisting an already ordered line is not exposed from this section; the existing shared ordered-row relist/void behavior remains available elsewhere.
...
- The MCP adapter freezes the spawning `initialResult` for first paint, checks `contract_version`, probes host capabilities, calls the app-callable snapshot read at boot, and enables controls only after successful re-hydration. Every successful persistent action uses `App.callServerTool`, consumes the returned authoritative snapshot and actual operation outcome, and immediately sends that entire `GroceryModelContext` through `ui/update-model-context`; check/add/remove/pantry/swap/relist produce no model turn, while **Mark order placed** additionally sends one `ui/message` carrying the same returned outcome. No client debounce, event delta, or intent-derived success message is used.
...
1. Add D1 columns/tables/indexes (`checked_at`, row version/timestamps, decision operation/ownership claims, row ownership, and `order_sends.placed_at`/placement claims) with null/default-compatible reads. Deploy Worker support before any new client bundle.
