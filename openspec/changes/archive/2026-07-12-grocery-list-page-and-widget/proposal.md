...
- Preserve the D16 boundary: list-page mark-placed materializes persisted send quotes and never re-prices; guarded per-line relist writes no spend for either a current open-send row or a row proven outside every open send, and whole-send and partial order results are reported from their actual operation outcomes. Manual-shop/walk completion and order-review refinements remain follow-on Band 3 changes.
