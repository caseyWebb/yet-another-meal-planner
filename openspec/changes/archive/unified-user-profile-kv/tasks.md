...
- [x] 14.4 Drop the build validators for files that moved to KV: remove `validateReadyToEatCatalog` and `validateKitchenInventory` from `scripts/build-indexes.mjs` (+ their `run()` call sites and the `READY_TO_EAT_MEALS`/`READY_TO_EAT_STATUSES` constants), and strip the `meal_plan` half of `validateCookingArtifacts` (keep `cooking_log`). These files are validated only by the Worker (`src/validate.ts`) at write time now. Update `tests/build-indexes.test.mjs` accordingly.
- [x] 14.5 Update docs for the dropped build validators (no-drift): `docs/SCHEMAS.md` validation-locations note + the kitchen/meal-plan build-failure references; `docs/ARCHITECTURE.md` "Indexes and validation" section.
