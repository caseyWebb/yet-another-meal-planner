...
- `recipe-metadata-contract`: `season` becomes a **controlled vocabulary** (`spring | summer | fall | winter`) enforced at write and build time (off-vocab hard-fails, like `requires_equipment`), with read-side normalization for legacy data; presence/empty-array rules are unchanged.
...
- **Season vocab enforcement:** the `season` check in `src/recipe-contract.js` (both gates) + classifier guidance in `AGENT_INSTRUCTIONS.md`. Any pre-existing non-canonical `season` values in the data repo are corrected before the build runs under the gate (read-side normalization covers reads in the meantime).
