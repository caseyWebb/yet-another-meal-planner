...
**6. `feedFetchMaxPerTick` is a `DEFAULT_CONFIG` constant (not an operator knob), calibrated against the *residual* budget.**
It is modeled on `retryFetchMaxPerTick` — a fetch **sub-budget** that is a pure `DEFAULT_CONFIG` const, deliberately *not* in the D1 `discovery_config` override or the admin calibration UI (unlike the primary `fetchMaxPerTick` lever, which is). A feed cap is a platform-budget guardrail, not a taste knob; keeping it a const avoids a D1 migration and Elm SPA work for a set-once value, and matches how the existing retry sub-budget is handled. The binding constraint is `flyer(~25) + recipe-page(fetchMaxPerTick=16) + feed(K) ≤ ~50`, i.e. `K ≲ 9`; default **`K = 6`** drains a dozen feeds in 2 ticks (10 min) and 24 in 4 ticks (20 min) — far fresher than RSS needs. The `DEFAULT_CONFIG` comment records this math so a future bump to `fetchMaxPerTick`/`flyerBatchUnits` cannot silently reopen #54. (Alternative — full parity with `fetchMaxPerTick`: D1 column + `loadDiscoveryConfig`/`saveDiscoveryConfig` + PUT validation + an Elm `knobRow` — was considered and rejected as over-scoped for a budget guardrail.)
