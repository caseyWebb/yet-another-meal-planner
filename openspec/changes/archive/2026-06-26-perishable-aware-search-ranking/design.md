...
**Saturating, peer-magnitude boost.** `pantryWeight` defaults in the 0.1–0.15 band, peer to `favoriteWeight` (0.15) and `noveltyBoost` (0.1), and the overlap saturates at `OVERLAP_CAP` so a recipe matching five items can't swamp cosine. Starting point: `W_PERISH = 1.0`, `W_KEY = 0.4`, `OVERLAP_CAP = 2` perishable-equivalents, `pantryWeight = 0.12`. These are re-rank tuning constants, adjustable without a contract change. **Shipped as fixed constants, not a per-tenant preference** — unlike `rotation`, which is a documented `update_preferences` key + `docs/SCHEMAS.md` column. A `pantry` pref mirroring `rotation` would force a preferences-contract change this change deliberately scopes out (and a half-measure is useless: `update_preferences` rejects unknown top-level keys, so an undocumented knob couldn't be set). `resolveRankParams` attaches the constants and carries a comment marking where a future `pantry` pref would slot in if tuning demand appears.
