...
> Implementation note (D6): the chosen capture signal is an explicit **`substitutes_for` annotation on `add_to_grocery_list`** (option 2a). The `place_order`-override path was considered but rejected — an override pins a raw SKU on the same line, yielding no clean replacement id Y without unreliable product re-normalization, and the same-SKU pairing is already claimed as the co-resolution *merge* signal. Annotating the accept write instead gives both canonical ids atomically and deterministically. The capture hooks the shared grocery write path (`session-db.ts` `addGroceryRow`), so it is surface-agnostic: the agent passes `substitutes_for` today; the member web app's one-tap accept is a future trigger onto the same path (held out of this change to avoid the app + Playwright surface). The cook log ("what did you actually use") remains a complementary future source.
