...
2. **Verify the Access JWT in the Worker, and turn off `workers.dev`.** Edge enforcement alone leaves the `*.workers.dev` hostname as a bypass of the custom-domain Access app, so the Worker validates `Cf-Access-Jwt-Assertion` (signature against the team JWKS, `aud === ACCESS_AUD`, issuer) on every `/admin*` request, and `wrangler.jsonc` sets `workers_dev: false` (already a merged code-level key) so the Access-protected custom domain is the only route in. Use **`jose`** (Web-Crypto build, workerd-safe, Cloudflare-documented for exactly this) over a hand-rolled RS256 verify — vetted code on a security-critical path. JWKS is cached in `KROGER_KV` with a short TTL.
...
6. **Elm SPA, committed `admin/dist/`, served via the `assets` binding, same-origin.** Same-origin serving means **no CORS** at all (it only arises cross-origin). Elm compiles in code-repo CI via `scripts/build-admin.mjs` (deterministic + `--check`, like `build-plugin.mjs`) into a **committed** `admin/dist/`, so the data-repo deploy needs no Elm toolchain — it just serves static files (the `plugin/` committed-generated precedent). `wrangler.jsonc` gains `assets` (directory `admin/dist`, bound) with `/admin/api/*` routed worker-first so the API reaches the handler instead of a static 404; **`assets` is added to the `merge-wrangler-config.mjs` allowlist** or it is silently dropped from operator deploys (the same trap that once shipped the `ai` binding undeployed). The committed minified bundle is non-reviewable in diffs; the `--check` drift gate (CI) and the authored `admin/src/**` source of truth mitigate it, and the `.map` is gitignored.
...
- **[Committed minified Elm blob]** is opaque in review. **Mitigation:** `--check` drift gate + authored source in `admin/src/**`; treat `admin/dist/` like `plugin/` (never hand-edited).
