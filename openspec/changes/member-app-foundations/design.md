...
*Implementation notes (recorded during the build, 2026-07):* the install surfaced one build-gated dep — `@parcel/watcher` (a `@tanstack/router-plugin` transitive) — pinned to its prebuilds with `allowBuilds: false`. Separately, aube's `no-downgrade` trustPolicy rejected `@trickfilm400/rollup-plugin-off-main-thread@3.0.0-pre1` (workbox-build 7.4's dep; its 3.x pre-release dropped the provenance its 2.x carried), resolved with a package-scoped `trustPolicyExclude` documented in `pnpm-workspace.yaml` — a trust exception, not a policy change. And `aube install` did not discover the two new workspace importers without `--force`, so the lockfile was re-resolved in one pass (all versions stayed within their declared ranges).

### 13. `packages/ui`: raw-TS workspace exports + the theme stylesheet

`@grocery-agent/ui` exports raw TS (`"exports": { ".": "./src/index.ts", "./theme.css": "./src/theme.css" }` — the `@grocery-agent/contract` precedent; the app's bundler compiles it). P0 contents: the `cn` util, the Tailwind v4 `@theme` token stylesheet translated from the design bundle (`docs/plans/web-app-design/`, Basecoat↔shadcn tokens map near-1:1), and the shadcn/ui primitives the P0 screens need (Button, Input, Card, Label) vendored as shadcn source (`components.json` lives in `packages/ui`; P0 vendored the four components' canonical Tailwind-v4/React-19 source by hand — the CLI's interactive init assumes an app-framework project and network access this environment lacked — so later additions can run the CLI against the committed `components.json`). The app's entry CSS: `@import "tailwindcss"; @import "@grocery-agent/ui/theme.css";` + `@source` directives covering both the app's `src/` and the ui package's `src/` so utility classes used inside shared components are compiled.
