...
- [ ] 1.1 In `packages/worker/src/discovery-classify.ts`, extend `SYSTEM_PROMPT` with a `title`
...
- [ ] 1.2 Anchor with few-shot exemplars: retitle ONE existing exemplar's input to a flowery form
...
- [ ] 1.3 Add the pure guard `cleanedTitleOrFallback(raw: string, cleaned: unknown): string` —
      ...
      by design).
- [ ] 1.4 Unit tests (`test/discovery-classify.test.ts` or sibling): guard accepts removal +
...
- [ ] 2.1 In `packages/worker/src/discovery.ts`, derive the default slug from the title with any
...
- [ ] 2.2 Unit tests: "Jatjuk (Pine Nut Porridge)" → `jatjuk`; "Beer Can Chicken" →
...
- [ ] 3.1 In `packages/worker/src/discovery-sweep.ts` `importRecipe`, catch the `slug_exists`
...
- [ ] 3.2 Confirm (test) the cleaned title flows end-to-end: a sweep-imported candidate with a
...
- [ ] 3.3 Sweep unit tests (fake deps): collision → `-2` suffix imported (not parked); 9-deep
      exhaustion → parked `error`; `create_recipe` (via `discovery-tools`) still returns
      `slug_exists` unchanged.

## 4. D1 + the title re-audit pass

- [ ] 4.1 Migration `packages/worker/migrations/d1/0042_title_audit.sql` (next available):
...
- [ ] 4.2 D1 accessors (via `src/db.ts`, throw-free): `loadTitleAuditBacklog(env, limit)`
...
- [ ] 4.3 `packages/worker/src/title-audit.ts`: the pass logic split from I/O (injected deps, the
      ...
      or path.
- [ ] 4.4 Wire `runTitleAuditJob(env, buildTitleAuditDeps(env, corpus))` into `scheduled()`
...
- [ ] 4.5 Born-stamping: after a successful write in the sweep's `importRecipe` and in
      `create_recipe` (`discovery-tools.ts`), best-effort `stampTitleAudit(…, outcome: 'kept')`
      (catch + log, never fail the committed import).
- [ ] 4.6 Unit tests (in-memory R2/D1 fakes): flowery title rewritten + stamped `cleaned` with
...
- [ ] 5.1 Test: a title rewrite changes the recipe-derived `content_hash` (title is in its
      domain, `recipe-embeddings.ts`) so the describe pass regenerates; the facet gate hash is
      unchanged (no reclassification).
- [ ] 5.2 Run `aubr typecheck` and the full `aubr test`; run `aubr test:tooling` if any shared
      helper moved.

## 6. Docs in lockstep (current-state voice, no history narration)

- [ ] 6.1 `docs/SCHEMAS.md` — the `title_audit` table (columns, the one-shot stamp semantics,
      born-stamped writes).
- [ ] 6.2 `docs/ARCHITECTURE.md` — the title re-audit in the scheduled-job list (bounded,
      one-shot-stamped, quiescing; slug immutability).
- [ ] 6.3 `docs/TOOLS.md` + the `create_recipe` tool description — slug derives from the dish
