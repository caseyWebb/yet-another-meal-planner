...
- **THEN** it loads the tenant forecast through the shared weather operation itself, and the member app reads the same operation via `GET /api/propose/weather` — no forecast tool appears on the MCP surface
