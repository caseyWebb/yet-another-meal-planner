...
`list_proposals`, `confirm_proposal`, `reconcile_read_signals`, `reconcile_enqueue_proposal`, and `update_recipe` register only when `ctx.operator`. Members confirm reconcile proposals in the web app's reconciliation queue (`member-app-core`, already shipped). `update_recipe` rides the operator plane because the confirmed `merge_recipes` flow (`recipe-dedup`) is agent-guided through it (fold content, re-point `pairs_with`, write `duplicate_of`) and stays operator-chat until an admin screen exists. This is "hard removal" from the *member* surface while keeping the recipe-dedup contract satisfiable. Call-time `isOperator` checks remain on the reconcile pair as defense in depth.
...
- **[Merge review depends on operator-plane `update_recipe` indefinitely]** → Scoped stopgap; the future admin merge screen removes it. The registration table documents it as operator-only so it never re-leaks to members.
...
None. (Resolved during planning: `mark_pantry_verified` is already `update_pantry`'s `verify` op; propose already loads weather server-side; the attention block needs exactly one new column; the app plane mechanism already exists as ext-apps visibility; merge review's `update_recipe` dependency is handled by the operator plane.)
