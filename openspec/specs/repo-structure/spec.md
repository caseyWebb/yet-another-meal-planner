...
The code repository (`caseyWebb/groceries-agent`) SHALL hold the Worker source under `src/` (including the operator admin panel — a Hono SSR + islands app — under `src/admin/`), the build tooling under `scripts/` (including the plugin builder `scripts/build-plugin.mjs` and the admin island builder `scripts/build-admin.mjs`), the agent persona source `AGENT_INSTRUCTIONS.md`, the D1 schema migrations under `migrations/d1/`, the reference docs under `docs/`, the OpenSpec workspace under `openspec/`, and the test suites under `test/` (Worker) and `tests/` (build tooling). Reusable CI workflows that operator data repos call SHALL live under `.github/workflows/`. The code repository SHALL NOT commit a generated `plugin/` bundle, a `.claude-plugin/marketplace.json`, or the built admin island bundle (`admin/dist/`); the plugin bundle and the admin bundle are generated by the operator's deploy (the plugin into the operator's data repo, the admin bundle into the Worker's static assets), not committed in the code repository (see "One data repository as the deploy control plane and plugin marketplace host" and `agent-plugin-distribution`).

#### Scenario: Worker source and tooling are present

- **WHEN** the code repository is cloned and listed
- **THEN** `src/`, `scripts/` (with `build-plugin.mjs`), `migrations/d1/`, `docs/`, `openspec/`, `test/`, and `tests/` are present along with `AGENT_INSTRUCTIONS.md`, and no committed `plugin/` bundle, `.claude-plugin/marketplace.json`, or `admin/dist/` bundle exists in the code repository

#### Scenario: The admin island bundle is built, not committed

- **WHEN** the admin panel is deployed (or run locally)
- **THEN** `admin/dist/` (the island bundles + stylesheet) is generated fresh from `src/admin/client/` (+ `src/admin/styles.css`) by `scripts/build-admin.mjs` and is NOT committed (it is gitignored); the server-rendered pages are produced by the Worker at request time, not pre-built
