# ai-usage-attribution Specification

## Purpose

The per-AI-call **attribution** tier — a third sibling of `usage-trends` (per-job) and `tool-usage-trends` (per-tool). Every `env.AI.run` inference routes through a single `src/ai.ts` gateway (`runAi`) that emits one tenant-clean Analytics Engine data point per call (activity, model, trigger, outcome, duration, calls, tokens, estimated neurons) to the `yamp_ai` dataset, at zero KV/D1 cost and below the embedding cache. The operator Usage page reads them back as a per-activity view — ranked neuron spend by activity, split by cron/import/request trigger, reconciled against the account-level by-model actual, and paired with job backlog — surfacing *which* of the Worker's activities spends the neurons that Cloudflare's account-level analytics reports only by model.
