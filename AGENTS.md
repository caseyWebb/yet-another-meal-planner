...
- [AGENT_INSTRUCTIONS.md](packages/plugin/AGENT_INSTRUCTIONS.md) - member-facing agent persona source; not auto-loaded into dev sessions.
...
- `AGENT_INSTRUCTIONS.md` is the persona source, owned by the `packages/plugin` workspace package. The plugin bundle is generated by `packages/plugin/scripts/build-plugin.mjs`; never hand-edit generated plugin output.
...
- The deploy merges wrangler configs via an allowlist in `packages/worker/scripts/merge-wrangler-config.mjs`; new binding types must be added explicitly.
