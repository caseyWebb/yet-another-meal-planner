...
`packages/scraper/package.json` `version` is the scraper's version — the value the running scraper reports to the Worker as `scraper_version` (stamped on every ingest batch) and the value the release publishes under. A PR that touches `packages/scraper/**` **or** the shared `packages/contract/**` (a contract change reshapes the scraper) must bump that `version` to a **strictly-greater** semver. The `scraper-version` gate in `ci.yml` (PR-only, bot-exempt) diffs against the PR base and fails the PR otherwise; it never commits the bump — you bump it in your PR. Like the other gates, it blocks merge only once `scraper-version` is added to `main`'s branch protection as a required status check.

**Releasing is automatic on merge.** You don't push a `scraper-v*` tag: on a push to `main`, `ci.yml`'s `detect-scraper-version` job compares the version against the previous commit, and when it changed, `release-scraper` calls the reusable `scraper-release.yml` **inline** (gated on green `test` + `no-open-changes`). That workflow reads/verifies the version from `packages/scraper/package.json`, builds the multi-arch (`linux/amd64` + `linux/arm64`) image, pushes it to GHCR (`ghcr.io/<owner>/groceries-scraper:<version>` + `:latest`), and cuts the `scraper-v<version>` GitHub Release — the tag is derived from `package.json` and created as part of the Release, all with the built-in `GITHUB_TOKEN` (no stored secret, no commit-back). So bumping the version in your PR is what publishes the release. The publish is idempotent (it skips a version whose `scraper-v<version>` release already exists), so a re-run or an unrelated push can't double-publish. Manual fallback: run the `scraper-release` workflow via `workflow_dispatch` — it publishes whatever version `package.json` currently declares (subject to the same idempotence guard). This release control plane is independent of the Worker deploy: a scraper version bump never deploys the Worker, and a Worker deploy never publishes a scraper image.
