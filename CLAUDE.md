...
- **[ARCHITECTURE.md](ARCHITECTURE.md)** — how the system is built: the determinism boundary, multi-tenant identity, the data model, the Kroger matching pipeline.
...
- **`docs/TOOLS.md` is the tool contract and `docs/SCHEMAS.md` the data-file contract** — when a tool's params/returns or a file's shape changes, update the matching doc in the *same* pass. No drift. An architectural shift updates `ARCHITECTURE.md`.
- **There is no data at the root of this repo.** Data lives in a separate private data repo; `users/<username>/` is the tenant prefix. See [`ARCHITECTURE.md`](ARCHITECTURE.md).
