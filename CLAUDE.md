...
- **The recipe index is projected by the Worker reconcile, not CI:** `src/recipe-projection.ts` reads the R2 corpus, validates it, and rebuilds the D1 `recipes` table each cron tick. One-time git→R2 corpus migration: `node scripts/migrate-corpus-to-r2.mjs --root /path/to/data-repo` (`--check` to list, `--verify` for R2↔git parity).
