...
const RecipeDetailPage = ({ detail }: { detail: RecipeDetail }) => (
  <DataShell active="recipes">
    ...
    {detail.body ? <div class="card-legacy" dangerouslySetInnerHTML={{ __html: md(detail.body) }} /> : null}
...
const GuidanceBrowser = ({ view }: { view: GuidanceView }) =>
  view.kind === "object" ? (
    <div>
      ...
      {view.markdown.trim() ? <div class="card-legacy" dangerouslySetInnerHTML={{ __html: md(view.markdown) }} /> : <p class="muted">(empty object)</p>}
    </div>
  ) : (
    <div>
